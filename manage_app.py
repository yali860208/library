...
def manager():
    return render_template('manage_base.html',title='Mangager')


class InputForm(FlaskForm):
	id_word = StringField('Id',validators=[DataRequired()])
	name_word = StringField('Book',validators=[DataRequired()])




@app.route('/append_book', methods = ['GET', 'POST'])
def append_book():
    form = InputForm()
    return render_template('manager_input.html',form=form,title='新增書目')


@app.route('/append_book_solution', methods=['POST'])
def append_book_solution():
	#book_db = get_book_db()
	...
	append_book_id = request.form.get('id')
	append_book_name = request.form.get('name')
	...
	append_book_sql = "SELECT book_id FROM booklist WHERE book_id = '{bookid}'".format(bookid=append_book_id, )
	
	select_appendbook_id= book_db.execute(append_book_sql)
	print('append_book',select_appendbook_id)
